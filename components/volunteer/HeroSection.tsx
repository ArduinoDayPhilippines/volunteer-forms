...
export function HeroSection({
  ...
  // Calculate smooth 3D perspective rotation and translation matching arduinodayphilippines.cc
  const rotateY = isDesktop && typeof window !== 'undefined'
    ? ((mouse.x - window.innerWidth / 2) / (window.innerWidth / 2)) * 14
    : 0
  const rotateX = isDesktop && typeof window !== 'undefined'
    ? -((mouse.y - window.innerHeight / 2) / (window.innerHeight / 2)) * 14
    : 0
  const translateX = isDesktop && typeof window !== 'undefined'
    ? (mouse.x - window.innerWidth / 2) * 0.02
    : 0
  const translateY = isDesktop && typeof window !== 'undefined'
  ...
  return (
    <section className="relative pt-8 pb-16 md:pt-14 md:pb-24 overflow-hidden">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* Main Hero Grid: Content on Left, 3D Perspective Logo on Right */}
        <div className="grid items-center gap-8 lg:grid-cols-[1.15fr_0.85fr] lg:gap-12 mb-14">
          ...
          <div className="flex flex-col items-center text-center lg:items-start lg:text-left order-2 lg:order-1">
            ...
            {/* Headline */}
            <h1
              className="text-3xl sm:text-5xl lg:text-6xl font-bold tracking-tight max-w-2xl leading-[1.12]"
          ...
          {/* 3D Perspective Logo Container */}
          <div className="flex items-center justify-center order-1 lg:order-2 py-4 lg:py-0">
            <div
              className="relative flex items-center justify-center pointer-events-none"
              style={{
                transform: `perspective(600px) rotateX(${rotateX}deg) rotateY(${rotateY}deg) translateX(${translateX}px) translateY(${translateY}px)`,
                transition: 'transform 0.1s ease-out',
              }}
            >
              {/* Radial Blur Aura Glow */}
              <div
                className="absolute inset-0 m-auto h-40 w-40 sm:h-52 sm:w-52 md:h-64 md:w-64 lg:h-80 lg:w-80 rounded-full blur-3xl"
                style={{
                  background: 'radial-gradient(circle, rgba(0,151,157,0.22) 0%, rgba(242,103,39,0.10) 65%, transparent 85%)',
                }}
                aria-hidden="true"
              />

              {/* 3D Clay Badge Emblem */}
              <div className="relative w-40 h-40 sm:w-52 sm:h-52 md:w-60 md:h-60 lg:w-72 lg:h-72 xl:w-80 xl:h-80 drop-shadow-[0_15px_30px_rgba(0,151,157,0.22)]">
                <Image
                  ...
                  priority
                  unoptimized
                  className="object-contain select-none [image-rendering:-webkit-optimize-contrast]"
                />
              </div>
            </div>
          </div>
        ...
        <div
          className="w-full max-w-4xl mx-auto rounded-xl border p-4 sm:p-5 text-left grid grid-cols-2 md:grid-cols-4 gap-4 mb-12"
          style={{
            background: 'var(--bg-card)',
            borderColor: 'var(--border-base)',
          }}
